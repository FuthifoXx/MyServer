...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    httpOnly: true // cookie can not be modified or accessed in any way by the browser prevents cross site scripting attacks
  };
  if (process.env.NODE_ENV === 'production') cookieOptions.secure = true; //https

  //Defining and sending a cookie
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forgot your password? Submit an UPDATE request with your new password and passwordConfirm to: ${resetURL}.\nIf you didn't forget your password , please ignore this email!`;
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  //NB! const user = await User.findAndUpdate() will not work as intended!
  ...
  // 4) Log user in, send JWT  createSendToken(user, 200, res);
  createSendToken(user, 200, res);
});
